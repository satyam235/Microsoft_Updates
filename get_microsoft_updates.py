...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Usage: %s <search string>" % os.path.basename(sys.argv[0]))
        sys.exit(1)

    search = sys.argv[1]
    for update in find_updates(search, all_updates=True, sort=None, sort_reverse=False):
        print("Found update: %s" % str(update))
        print("MSRC Number: %s" % update.msrc_number)
        print("MSRC Severity: %s" % update.msrc_severity)
        print("Support URL: %s" % update.support_url)
        # print("  KB Number: %s" % update.kb_number)
        print("Details: %s" % update.description)
        print("Architecture: %s" % update.architecture)
        print("More Info : %s" % update.more_information)
        print("Uninstallable : %s" % update.is_installable)
        print("Download URLs:")
        if len(update.get_download_urls())==0:
            print("No Download URL available")
        if len(update.get_download_urls())>0:
            print("First Download URL: %s" % str(update.get_download_urls()[0].url))

        # for download in update.get_download_urls():
        #     print("%s" % str(download.url))
